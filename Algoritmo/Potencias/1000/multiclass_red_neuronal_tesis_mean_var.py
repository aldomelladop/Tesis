...
#  
df1 = fixrows('Potencia_r1').iloc[:1000,:]
...
# 
df2 = fixrows('Potencia_r2').iloc[:1000,:]
...
df3 = fixrows('Potencia_r3').iloc[:1000,:]
...
df4 = fixrows('Potencia_r4').iloc[:1000,:]
...
classifier = KerasClassifier(build_fn = build_classifier, batch_size = 64, epochs = 200)
accuracies = cross_val_score(estimator = classifier, X = X_trainn, y = y_train, cv = 15, n_jobs = -1)
mean = accuracies.mean()
variance = accuracies.std()

history = classifier.fit(X_trainn, y_train, batch_size = 32, epochs = 100, validation_split=0.2)
