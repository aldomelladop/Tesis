...
son = str(input("Desea estandarizar(s) o normalizar (n) = ")).capitalize()
...
#     classifier.add(Dropout(rate = 0.3))
# #6
#     classifier.add(Dense(units = np.shape(y_test)[1], kernel_initializer = 'uniform', activation = 'relu'))    
#     classifier.add(Dropout(rate = 0.3))
# #7
...
#     Move file to folder
# =============================================================================
mv = directory + '/{}_{}/Confusion_matrix.png'.format(j,son)
os.system('mv Confusion_matrix.png '+ mv)

mv = directory + '/{}_{}/Classification_report.csv'.format(j,son)
...
# history = classifier.fit(X_train, y_train, validation_split=0.2, epochs= best_parameters['epochs'], batch_size= best_parameters['epochs'], verbose=0)
...
# plt.savefig( os.getcwd() + '/{}_{}/accuracy_over_epochs_train_{}.pdf'.format(j,son,j))
